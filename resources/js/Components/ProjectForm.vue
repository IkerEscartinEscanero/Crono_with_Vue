...
// Si existe proyecto cojo los datos, sino será nulo
const form = useForm({
  name: props.project?.name ?? "",
  hours: props.project?.hours ?? 0,
  description: props.project?.description ?? "",
  start_date: props.project?.start_date ?? new Date(),
})

// Si tengo proyecto lo edito, si no tengo lo almaceno en la base de datos para mostrarlo en la tabla
const submit = () => {
  // Al hacer un store se utiliza POST (asegurar con php artisan route:list --name=project)
  if (props.project) {
  ...
  <div class="flex justify-center mt-10">
    <div class="card w-full max-w-2xl bg-base-100 shadow-xl">
      <div class="card-body">
        ...
        <!-- Cuando aprete submit, no emita lo que por defecto hace el form (enviar una solicitud) y que en su lugar ejecute el submit -->
        <form @submit.prevent="submit" class="space-y-4">
          ...
          <div class="form-control">
            ...
            />
            <label v-if="form.errors.start_date" class="label">
